<template>
    <modal-layout>
        <dialog_loader :dialog="loadingDialog"></dialog_loader>
                ...
                <v-form autocomplete="off" @submit.prevent="login"
                    ...
                    <v-stepper-content step="1">
                        ...
                        <v-layout pa-2 align-center justify-center>
                            <v-flex>
                                <tiptap_editor style="font-size: 16px;" v-model="form.long_description" :text="form.long_description" :label="$t('product_long_description')"></tiptap_editor>
                            </v-flex>
                        </v-layout>
                        <v-btn
                                :loading="loading"
    ...
    export default {
        ...
        methods: {
            ...
            getData() {
                ...
                ]).then(this.$http.spread((categoriesRes, tagsRes, productRes) => {
                    this.categories = categoriesRes.data;
                    this.tags = tagsRes.data;

                    this.form.fill(productRes.data);
                    this.form.category = _.map(productRes.data.categories, 'id');

                    this.form.product_images.push({'image':this.form.picture});
                    this.form.product_images = _.map(productRes.data.product_images, function (item) {
                        return `storage/products/${item.image}`;
                    });
                    this.form.tags2 = this.form.tags;
                    this.sizes = [...new Set(_.map(productRes.data.product_specs, 'size'))];
                    this.colors = [...new Set(_.map(productRes.data.product_specs, 'color'))];
                    this.package_quantities = [...new Set(_.map(productRes.data.product_specs, 'package_quantity'))];
                    // this.add_variations();
                    this.showTable = !!productRes.data.product_specs.length;
            ...
            submit() {
                if(!this.form.main_image){
                    snackbar(this.$i18n.t('please_choose_main_image'), 'error');
                    return;
                }
                this.loading = true;
                this.processTags();
